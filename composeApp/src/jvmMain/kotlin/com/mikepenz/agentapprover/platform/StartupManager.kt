...
object StartupManager {
    ...
    private val macPlistFile = File(macPlistDir, "com.mikepenz.agentapprover.plist")
    ...
    private fun findAppExecutable(): String {
        ...
        return if (classPath.isNotBlank()) {
            "$java -cp $classPath com.mikepenz.agentapprover.MainKt"
        } else {
...
<dict>
    <key>Label</key>
    <string>com.mikepenz.agentapprover</string>
...
""")
    ...
    private const val WIN_REG_NAME = "AgentApprover"
    ...
    private fun setWindowsStartup(enabled: Boolean) {
        ...
        if (enabled) {
            val command = if (classPath.isNotBlank()) {
                "\"$javaw\" -cp \"$classPath\" com.mikepenz.agentapprover.MainKt"
            } else {
                "\"$javaw\""
            }
    ...
    private val linuxDesktopFile = File(linuxAutostartDir, "agent-approver.desktop")
