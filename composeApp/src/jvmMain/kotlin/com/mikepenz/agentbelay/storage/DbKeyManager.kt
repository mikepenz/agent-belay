...
object DbKeyManager {
    ...
    private const val KEYRING_SERVICE = "com.mikepenz.agentbelay"

    // Legacy service names from prior brand renames. Probed in newest-first
    // order so users coming from Agent Buddy migrate in one hop, and users
    // coming from the original Agent Approver also migrate.
    private val LEGACY_KEYRING_SERVICES = listOf(
        "com.mikepenz.agentbuddy",
        "com.mikepenz.agentapprover",
    )
     ...
     * one does under any prior brand's service name, copy it over and remove
     * the legacy entry. Returns the migrated key, or null on any error /
     * absence. Probes services in [LEGACY_KEYRING_SERVICES] order.
     */
    private fun tryMigrateLegacyEntry(kr: Keyring): SecretKey? {
        for (legacyService in LEGACY_KEYRING_SERVICES) {
            val legacy = try {
                kr.getPassword(legacyService, KEYRING_ACCOUNT)
            } catch (_: PasswordAccessException) {
                continue
            } catch (_: Throwable) {
                continue
            ...
            val bytes = decodeKeyringValue(legacy) ?: continue
            return try {
                kr.setPassword(KEYRING_SERVICE, KEYRING_ACCOUNT, encodeKeyringValue(bytes))
                runCatching { kr.deletePassword(legacyService, KEYRING_ACCOUNT) }
                logger.i { "Migrated database key from legacy keyring entry ($legacyService)" }
